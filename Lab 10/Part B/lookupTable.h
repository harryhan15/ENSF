...
template<class K, class D> LookupTable<K, D>& LookupTable<K, D>::begin() {
  cursorM = headM;
  return *this;
}


template<class K, class D> LookupTable<K, D>::LT_Node::LT_Node(const Pair& pairA, LT_Node *nextA)
  : pairM(pairA), nextM(nextA) {}


template<class K, class D> LookupTable<K, D>::LookupTable()
  : sizeM(0), headM(0), cursorM(0) {}


template<class K, class D> LookupTable<K, D>::LookupTable(const LookupTable<K, D>& source) {
  copy(source);
}


template<class K, class D> LookupTable<K, D>& LookupTable<K, D>::operator =(const LookupTable<K, D>& rhs) {
  if (this != &rhs) {
    destroy();
    copy(rhs);
  }
...
template<class K, class D> int LookupTable<K, D>::size() const {
  return sizeM;
}


template<class K, class D> int LookupTable<K, D>::cursor_ok() const {
  return cursorM != 0;
}

template<class K, class D> const K& LookupTable<K, D>::cursor_key() const {
  assert(cursor_ok());
  return cursorM->pairM.key;
}

template<class K, class D> const D& LookupTable<K, D>::cursor_datum() const {
  assert(cursor_ok());
  return cursorM->pairM.datum;
}


template<class K, class D> void LookupTable<K, D>::insert(const Pair& pairA) {
...
template<class K, class D> void LookupTable<K, D>::remove(const K& keyA) {
...
template<class K, class D> void LookupTable<K, D>::find(const K& keyA) {
  LT_Node *ptr=headM;
...
template<class K, class D> void LookupTable<K, D>::go_to_first() {
  cursorM = headM;
}


template<class K, class D> void LookupTable<K, D>::step_fwd() {
  assert(cursor_ok());
  cursorM = cursorM->nextM;
}


template<class K, class D> void LookupTable<K, D>::make_empty() {
  destroy();
  sizeM = 0;
  cursorM = 0;
}

template<class K, class D> void LookupTable<K, D>::destroy() {
...
template<class K, class D> void LookupTable<K, D>::copy(const LookupTable<K, D>& source) {
...
template<class K, class D> const D& LookupTable<K, D>::Iterator::operator *() {
  assert(LT ->cursor_ok());
...
template<class K, class D> const D& LookupTable<K, D>::Iterator::operator ++() {
  assert(LT->cursor_ok());
  const D& d = LT->cursor_datum();
  LT->step_fwd();
  return d;
}

template<class K, class D> const D& LookupTable<K, D>::Iterator::operator ++(int) {
  assert(LT->cursor_ok());
...
template<class K, class D> int LookupTable<K, D>::Iterator::operator!() {
  return (LT->cursor_ok());
}
